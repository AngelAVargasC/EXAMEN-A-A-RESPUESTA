...
def dashboard(request):
    ...
    # Sitios urgentes (< 30 minutos)
    urgentes = [d for d in data if d['avg_backup'] is not None and d['avg_backup'] < 30]
    urgentes_labels = [d['site_parsed_alarm'] for d in urgentes]
    urgentes_values = [d['avg_backup'] for d in urgentes]

    context = {
        'labels': labels,
        'values': values,
        'urgentes_labels': urgentes_labels,
        'urgentes_values': urgentes_values,
    }
